...
USING (
        SELECT
            LEAD(EFFECTIVESTARDDATE, 1) OVER (PARTITION BY ID_ROLE_TIERS ORDER BY EFFECTIVESTARDDATE) AS DATE_SUIV,
            EFFECTIVESTARDDATE,
            EFFECTIVEENDDATE,
            ID_ROLE_TIERS
        FROM
...
ON (
    ...
    AND
    TARGET_TABLE.ID_ROLE_TIERS = SOURCE_TABLE.ID_ROLE_TIERS
)
